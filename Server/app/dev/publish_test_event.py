...
broadcast envelope ``{status, added_cities, event}`` and publishes it through the
normal ``publish_alert`` path - so every connected browser (local or cloud)
receives it exactly like a real Oref alert, without waiting for one.
...
def build_event(city: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "cities": [{"id": str(uuid.uuid4()), "name": city}],
    }


def main() -> None:
    city = _city_arg()
    event = build_event(city)
    payload = {"status": "created", "added_cities": [city], "event": event}
